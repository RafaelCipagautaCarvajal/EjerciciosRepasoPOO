...
case 1: 
srand(time(0)); 
for (int i=0;i<3;i++){ num="rand()%10;">0){ 
for (int j=0;j<<"Ingrese cada digito seguido de un enter"<>a[1][j]; 
} 
for (int x=0;x<3;x++){ fijas="=" cout=""><<"Ud tiene una puntuacion de: "<<10-h<<" h="10;" g="0;g<3;g++){" for="" int="" y="0;y<3;y++){" if="" cout=""><<"Usted tiene "<<<" fijas y "<<<" picas"<<<"Has perdido!!!!!"<<<"El numero era: "; for (int l=0;l<3;l++){ cout=""><0){ 
...
cout<<"Ud tiene una puntuacion de: "<<20-h<<"> 
h=20; 
} 
} 
